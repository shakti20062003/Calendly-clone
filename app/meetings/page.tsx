...
export default function MeetingsPage() {
  ...
  const fetchMeetings = async () => {
    try {
      ...
      // upcoming response
      setOwnerTimezone(upcomingRes.data?.ownerTimezone || 'UTC');
  ...
  const MeetingCard = ({ booking, showCancel }: { booking: Booking; showCancel: boolean }) => {
    // Get user's timezone
    const userTimezone = ownerTimezone;
              ...
              <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-gray-600">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-xs sm:text-sm">
                    {formattedStartTime} - {formattedEndTime} ({booking.duration} min)
                  </span>
                </div>
                <div className="flex items-center">
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent className="w-[95vw] sm:max-w-md">
          ...
          {selectedBooking && (() => {
            // Get user's timezone
            const userTimezone = ownerTimezone;
            const startTime = new Date(selectedBooking.start_time);
